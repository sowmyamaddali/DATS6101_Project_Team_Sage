...
trainrose<-ROSE(stroke~.,data=train)$data
...
# rf 1
```{r}
x = subset(trainrose, select = -c(age,ever_married,Residence_type))
test = subset(test, select = -c(age,ever_married,Residence_type))

```
```{r}
classifier_RF1 = randomForest(x = x[-8],
                             y = x$stroke,
                             ntree = 500)
...
```{r}
# Predicting the Test set results
y_pred = predict(classifier_RF1, newdata = test[-8])
