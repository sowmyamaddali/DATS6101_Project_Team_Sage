...
## Logistic Regression:
Logistic Regression is a type of statistical model (also known as logit model) is often used for classification and predictive analytics. Logistic regression estimates the probability of an event occurring, such as voted or didn’t vote, based on a given dataset of independent variables. Since the outcome is a probability, the dependent variable is bounded between 0 and 1.
...
Recall and Accuracy were selected as classification metrics for comparing the performance results of different models with respect to two classes.

• Class 0- No stroke Cases.

• Class 1- Stroke Cases.
...
• TN – True Negative : A true negative is an outcome where the model correctly predicts the negative class, here Patient has no stroke and model classified as having no stroke.

• TP – True Positive : A true positive is an outcome where the model correctly predicts the positive class, here Patient has a stroke and the model classifies it as a stroke.

• FP – False Positive : A false positive is an outcome where the model incorrectly predicts the positive class, here a Patient who does not have stroke is classified as a patient having a stroke.

• FN – False Negative : A false negative is an outcome where the model incorrectly predicts the negative class, here a Patient who has a stroke but is classified as no stroke.


# Handling Imbalanced Dataset:

A dataset is said to be imbalanced when the number of observations per class is not equally distributed among the training dataset.There has been a heavy class imbalance in our dataset where there is more number of patients with no stroke and less number of patients with stroke.
• Class 0 - No stroke patients-  cases.
• Class 1 - Stroke patients - cases.


To reduce the imbalance dataset three sampling methods are performed. The samplers are,
• Random OverSampler Examples
• SMOTE
...
### Tuned decision tree with feature selection - ROSE Technique
...
## Random Over Sampler Examples (ROSE)
...
ROC curve shows us the behavior of the classifier for every threshold by plotting two variables: the True Positive Rate (TPR) and the False Positive Rate (FPR).
