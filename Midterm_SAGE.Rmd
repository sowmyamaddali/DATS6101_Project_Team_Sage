...
ggplot(stroke_1, aes(x = age, fill=stroke))+ geom_density(alpha = 0.3)+ ggtitle("Density plot for stroke and age")
...
ggplot(dat, aes(x=bmi.cat,y = age, fill=stroke))+geom_boxplot()
...
ggplot(dat, aes(fill=stroke, y = age, x = avg_gluc.cat))+ geom_boxplot()
```
```{r}
...
ctest = chisq.test(as.factor(dat$Residence_type), dat$stroke)
ctest
