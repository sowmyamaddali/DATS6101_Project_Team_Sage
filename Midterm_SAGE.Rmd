...
dat <- within(stroke_df, {   
  ...
  age.cat[age >= 20 & age < 40] <- "From 20 to 39"
  age.cat[age >= 40 &age < 60] <- "From 40 to 59"
  age.cat[age >= 60 &age < 83] <- "From 60 to 82" # As maximum value is 82
   } )
...
dat_gluc <- within(stroke_df, {   
  ...
  avg_gluc.cat[avg_glucose_level >= 60 & avg_glucose_level < 90] <- "From 60 upto 90"
  avg_gluc.cat[avg_glucose_level >= 90 & avg_glucose_level < 140] <- "From 90 upto 140"
  avg_gluc.cat[avg_glucose_level >= 140 & avg_glucose_level < 273] <- "From 140 upto 273" 
   } )
...
```{r}
df = subset(stroke_df,stroke_df$work_type == "Private" | stroke_df$work_type == "Self-employed" | stroke_df$gender == "Male")
df


ggplot(df, aes(fill=work_type, y=gender, x=stroke )) + 
  geom_boxplot()
...
```{r}
#calculating the chi-square test for the ever_married column and the target coulmn that is stroke

ctest = chisq.test(as.factor(stroke_df$ever_married), stroke_df$stroke)
ctest
```
*As p value is 1.639e-14 then we can say ever_married feature is dependent as p value is < 0.05*

```{r}
# calculating the chi-square test for the age and stroke column to determine if age is a dependent or independent variable.

ctest1 = chisq.test(as.factor(stroke_df$age), stroke_df$stroke)
ctest1
```
```{r}
#calculating the chi-square test for the avg_glucose_level and stroke variable to check if average glucose level is a dependent or a independent variable.

ctest2 = chisq.test(as.factor(stroke_df$avg_glucose_level), stroke_df$stroke)
ctest2
```
*As the p value is <0.05 we can say that the average glucose level is a independent variable*
