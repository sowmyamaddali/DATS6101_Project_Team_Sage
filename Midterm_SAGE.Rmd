...
```
```{r}
ggplot(stroke_df, aes(x=stroke,fill=stroke))+geom_bar()+ggtitle("Distribution of Target varibale after sampling")+ theme_bw() +  theme()+ xlab("Stroke") + ylab("Count of people")
```
...
ggplot(stroke_df,aes(x=work_type, fill= smoking_status))+geom_bar(position = "dodge")+ggtitle("Smoking habbits in different work type")+ theme_bw() + theme()+ xlab("Work Type") + ylab("Count of people ")
```
...
dat$bmi.cat<- as.factor(dat$bmi.cat)
dat$avg_gluc.cat<- as.factor(dat$avg_gluc.cat)
str(dat)
...
ggplot(dat, aes(x=bmi.cat,y = age, color=stroke))+geom_boxplot()



ggplot(dat,aes(x= avg_gluc.cat, fill=stroke))+geom_bar(position = "dodge")+ggtitle("Avg Gulcose level Vs Stroke")+ theme_bw() +
  theme()+ xlab("Avg Glucose level") + ylab("Count of people who had a stroke")
#dist of glucose by work type
ggplot(dat,aes(y=avg_glucose_level , fill=work_type))+geom_boxplot()+ggtitle("Avg Gulcose level Vs Stroke")+ theme_bw() +
  theme()+ xlab("Avg Glucose level") + ylab("Count of people who had a stroke")
# we can see 25 % to 75 % who get stroke are from age 58 - 78 . the higher the age the chances of getting a stroke is more
# we can see become who have bmi from 27.5 to 32.5 have higher chances of stroke
```
...
```{r}
str(dat)
ctest = chisq.test(as.factor(dat$work_type), dat$stroke)
ctest
```
