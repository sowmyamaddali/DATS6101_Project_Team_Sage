...
#STROKE VS ABG_GLUCOSE_LEVEL


ggplot(stroke_df, aes(x = stroke, y = avg_glucose_level, group = stroke, fill = stroke)) + geom_boxplot()
...
ggplot(stroke_df, aes(x = factor(gender), fill = factor(gender))) + geom_bar() + theme_classic()
...
ggplot(stroke_df, aes(x = age, y = bmi, color = stroke)) + geom_point() 
...
superheat(subset(stroke_df, select = -c(as.numeric(stroke))), scale = TRUE, bottom.label.size = 0.5, bottom.label.text.angle = 90, bottom.label.text.size = 3)
...
library(ggplot2)
df<-subset(stroke_df,stroke_df$work_type=='Private'& stroke_df$stroke==1)
df

resi1=table(df$stroke,df$smoking_status)
barplot(resi1,
main = "Distribution of Resident People affected by Brain Stroke",
xlab = "Residence_type",
ylab = "Number of people",
...
stroke_df$smoking_status <- as.numeric(factor(stroke_df$smoking_status))
head(stroke_df)
...
ch1<-table(stroke_df$smoking_status, stroke_df$stroke)
chisq.test(stroke_df$smoking_status, stroke_df$stroke)

format.pval(pv =2.085e-06, 
...
ch2<-table(stroke_df$hypertension, stroke_df$stroke)  

chisq.test(stroke_df$hypertension, stroke_df$stroke)

format.pval(pv =2.2e-16, 
...
From 3 sampling techniques we can see BOTH ( UNDER + OVER ) is better compared to the rest.
We need to decide which one to take whether 0 or 1 if we focus on 0 then no need of sampling 
if we focus on achieving 1 BOTH(Under+over) is better when compare to the rest.
